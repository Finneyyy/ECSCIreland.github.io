...
Initially we got the payload from analysing Tulip logs. We noticed someone with the cookie `settings=isAdmin:1` could read any contact on the platform. The contact ids for the flags are given to us by the A/D API.
